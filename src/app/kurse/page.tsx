...
const title = 'Hundekurse Neusiedl am See – Gruppenkurse | Willenskraft';
const description =
  'Alle Gruppenkurse mit Jessy in der Region Neusiedl am See: Welpenkurs, Junghundekurs, Begegnungstraining, Dummytraining & Medical Training.';

export const metadata: Metadata = {
  title: { absolute: title },
  description,
  alternates: { canonical: 'https://welpenschule-neusiedl.at/kurse' },
  openGraph: { title, description, url: 'https://welpenschule-neusiedl.at/kurse', type: 'website', images: [{ url: 'https://welpenschule-neusiedl.at/og-image.jpg', width: 1200, height: 630, alt: 'Welpenschule Neusiedl am See – mobile Hundeschule Willenskraft' }] },
};

const itemListJsonLd = {
  '@context': 'https://schema.org',
  '@type': 'ItemList',
  name: 'Gruppenkurse der Hundeschule Willenskraft — Region Neusiedl am See',
  itemListElement: kurse.map((k, i) => ({
    '@type': 'ListItem',
    position: i + 1,
    name: k.name,
    url: `https://welpenschule-neusiedl.at/kurse/${k.slug}`,
  })),
};
      ...
      <section className="wk-section relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 max-w-4xl text-center relative">
          ...
          <p className="mt-7 text-lg md:text-xl text-ink-600 max-w-2xl mx-auto leading-relaxed">
            ...
            finden in Kleingruppen an sorgfältig ausgewählten <strong>Outdoor-Treffpunkten in deiner
            Region</strong> statt — gewaltfrei nach dem LIFE-Modell, mit positiver Verstärkung und deiner
            Trainerin Jessy.
          </p>
      ...
      <section className="container mx-auto px-6 pb-8 max-w-7xl">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <Link key={k.slug} href={`/kurse/${k.slug}`} className="block group">
              <article className="wk-card !p-0 overflow-hidden h-full flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <h2 className="text-xl font-semibold text-ink-950 group-hover:text-lake-700 transition-colors tracking-tight">
                    {k.name}
                  </h2>
                  ...
                  <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-ink-500">
                    <span className="inline-flex items-center gap-1"><Euro className="w-3.5 h-3.5 text-lake-600" /> {k.units}</span>
                    <span className="inline-flex items-center gap-1"><Users className="w-3.5 h-3.5 text-lake-600" /> {k.groupSize}</span>
                  </div>
                  <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-lake-700">
          ...
          <Link href="/mobiles-hundetraining" className="block group">
            <article className="h-full rounded-3xl p-6 bg-gradient-to-br from-brand-400 via-brand-500 to-lake-700 text-ink-950 shadow-[0_30px_60px_-20px_rgba(186,135,30,0.5)] flex flex-col">
              ...
              <p className="text-sm text-ink-950/80 leading-relaxed flex-1">
                Jessy kommt zu dir nach Hause — in alle 17 Orte des Bezirks. Erstberatung 95 €,
                Einzelstunde ab 55 €, Anfahrt kostenlos.
              </p>
