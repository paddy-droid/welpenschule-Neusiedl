...
// Universal training knowledge base — designed to be AI-scrapeable per region — Neusiedl-specific climate
const trainingKnowledgeBase = (regionName: string) => [
  {
    question: 'Wie oft pro Woche sollte ich mit meinem Hund trainieren?',
    answer: `Optimal sind 3–5 kurze Einheiten pro Woche à 5–10 Minuten plus Alltagsübungen. Mehr ist nicht besser – Hunde lernen in kurzen, fokussierten Sessions deutlich nachhaltiger als in langen Trainingsblöcken. Im mobilen Training in ${regionName} bauen wir gemeinsam einen realistischen Trainingsplan, der zu deinem Alltag passt.`,
    icon: Clock,
  },
  {
    question: 'Wie lange dauert es, bis ein neues Signal sitzt?',
    answer:
      'Ein einzelnes Signal kann nach 50–100 sauberen Wiederholungen unter ablenkungsarmen Bedingungen abgerufen werden. Für Generalisierung (Signal funktioniert überall) braucht es deutlich mehr Wiederholungen unter wechselnden Bedingungen — typischerweise 4–8 Wochen.',
    icon: Activity,
  },
  {
    question: 'Welche Belohnung wirkt am Neusiedlersee am besten?',
    answer:
      'Hochwertige Futterbelohnungen (Käse, Fleisch, Leberwurst) wirken bei den meisten Hunden am stärksten — gerade bei der hohen Reizdichte am See ist es wichtig, eine Belohnung zu wählen, die mit Vögeln und Wind konkurrieren kann. Spielzeug und Sozialkontakt sind zusätzliche Optionen für besonders reaktive Hunde.',
    icon: Heart,
  },
  {
    question: 'Wann ist die beste Tageszeit zum Trainieren?',
    answer: `Im pannonischen Klima rund um ${regionName} mit über 300 Sonnentagen empfehlen wir im Sommer Training früh morgens (bis 9 Uhr) oder abends (ab 18 Uhr). Asphalt-Test mit dem Handrücken: Hält man 5 Sekunden ohne Schmerz aus, ist es auch für Hundepfoten ok. Im Winter sind Mittagsstunden ideal.`,
    icon: Sun,
  },
  {
    question: 'Wie gehen wir mit Wildvogel-Reizen am See um?',
    answer:
      'Der Schilfgürtel und die Lacken sind voller Wildvögel — eine echte Herausforderung. Wir arbeiten mit Distanz-Training (zuerst weit weg von den Reizen), Impulskontrolle und Alternativ-Verhalten. Innerhalb 4–8 Sitzungen sehen die meisten Halter deutliche Fortschritte. Schlüssel: konsequentes Üben im Alltag.',
    icon: Bird,
  },
  {
    question: 'Wie viel Schlaf braucht ein Welpe?',
    answer:
      'Welpen brauchen 18–22 Stunden Schlaf täglich. Schlafmangel ist eine häufige Ursache für Beißhemmungsprobleme, Übermüdung und scheinbar „aggressives" Verhalten. Ein eigener, ruhiger Rückzugsort ist Pflicht — gerade in Urlaubsregionen wie der unsrigen, wo viel Trubel herrscht.',
    icon: Snowflake,
  },
];
...
export default function RegionPageTemplate({ regionKey }: { regionKey: string }) {
  ...
  const breadcrumbsJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Mobiles Hundetraining', item: 'https://welpenschule-neusiedl.at/mobiles-hundetraining' },
      { '@type': 'ListItem', position: 3, name: data.name, item: `https://welpenschule-neusiedl.at/mobiles-hundetraining/${data.slug}` },
    ],
  };
            ...
            <div className="wk-card p-8 border-l-[3px] !border-l-rose-400">
              ...
              <div className="space-y-5">
                {[
                  ...
                  { title: 'Vogel- &amp; Wildjagd', text: 'Der Seewinkel mit seiner Vogelvielfalt ist Belastungstest für jeden Hund.' },
                ].map((item) => (
                  <div key={item.title} className="flex items-start gap-3">
                    <div className="w-2 h-2 mt-2 rounded-full bg-rose-400 shrink-0" />
                    <div>
                      <h4 className="font-semibold text-ink-950 mb-1" dangerouslySetInnerHTML={{ __html: item.title }} />
            ...
            <div className="wk-card p-8 border-l-[3px] !border-l-emerald-500">
              ...
              <div className="space-y-5">
                {[
                  ...
                  { title: 'Impulskontrolle', text: 'Spezielles Training für Wildvogel-Reize am Neusiedlersee.' },
                ].map((item) => (
                  <div key={item.title} className="flex items-start gap-3">
      ...
      <section className="wk-section">
        <div className="container mx-auto px-6 max-w-4xl">
          <div className="text-center mb-14">
            ...
            <h2 className="wk-display text-4xl md:text-5xl text-ink-950">Häufige Fragen aus {data.name}.</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">
            {data.regionHighlights.map((highlight) => {
              const iconMap: Record<string, React.ReactNode> = {
                ...
                reed: <Leaf className="w-5 h-5" />,
                wine: <Grape className="w-5 h-5" />,
                'national-park': <Bird className="w-5 h-5" />,
                historic: <Landmark className="w-5 h-5" />,
                field: <Wheat className="w-5 h-5" />,
                bird: <Bird className="w-5 h-5" />,
                cycle: <Bike className="w-5 h-5" />,
                border: <Compass className="w-5 h-5" />,
                cellar: <Grape className="w-5 h-5" />,
                church: <Landmark className="w-5 h-5" />,
              };
              return (
                <div key={highlight.title} className="wk-card p-7 group">
                  ...
                  <h3
                    className="text-lg font-semibold text-ink-950 mb-3 tracking-tight"
                    dangerouslySetInnerHTML={{ __html: highlight.title }}
                  />
                  <p
                    className="text-ink-600 leading-relaxed text-sm"
                ...
                <tbody>
                  {data.regionTableRows.map((row, idx) => (
                    ...
                    >
                      <td className="px-7 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            ...
                            dangerouslySetInnerHTML={{ __html: row.trainingsort }}
                          />
                        </div>
                      </td>
                      <td className="px-7 py-5">
                      ...
                      <td className="px-7 py-5 text-ink-700 text-sm" dangerouslySetInnerHTML={{ __html: row.besonderheit }} />
              ...
              {data.regionTableRows.map((row, idx) => (
                <div key={idx} className="wk-card p-6">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      <h4
                        className="font-semibold text-ink-950 text-base"
                        dangerouslySetInnerHTML={{ __html: row.trainingsort }}
                      />
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <p className="text-ink-700 text-sm" dangerouslySetInnerHTML={{ __html: row.besonderheit }} />
                    </div>
                    <div>
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="text-center mb-14 max-w-3xl mx-auto">
            ...
            <p className="mt-6 text-lg text-ink-600 leading-relaxed">
              Geo­daten, Klima­fakten und Trainings­besonderheiten — kompakt für Suchmaschinen, KI-Crawler und Hundehalter.
            </p>
          </div>
            ...
            <div className="md:col-span-5 space-y-4">
              <div className="wk-card p-6">
                ...
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex items-baseline justify-between gap-3 pb-3 border-b border-ink-200/70">
                    <dt className="text-ink-500">Bundesland</dt>
                    <dd className="font-semibold text-ink-950 text-right">Burgenland</dd>
                  </div>
                  <div className="flex items-baseline justify-between gap-3 pb-3 border-b border-ink-200/70">
                    <dt className="text-ink-500">Klima</dt>
                    <dd className="font-semibold text-ink-950 text-right">Pannonisch</dd>
                  </div>
                  <div className="flex items-baseline justify-between gap-3 pb-3 border-b border-ink-200/70">
                    <dt className="text-ink-500">Sonnentage/Jahr</dt>
                    <dd className="font-semibold text-ink-950 text-right">~ 300</dd>
                  </div>
                  <div className="flex items-baseline justify-between gap-3 pb-3 border-b border-ink-200/70">
                ...
                <ul className="space-y-3 text-sm">
                  <li className="flex items-start gap-2.5">
                    ...
                    <span className="text-ink-700">
                      <strong className="text-ink-950">Sommer:</strong> Über 30 °C möglich — Training früh morgens oder ab 18 Uhr. Asphalt-Test mit Handrücken.
                    </span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <Snowflake className="w-4 h-4 mt-0.5 text-lake-600 shrink-0" />
                    <span className="text-ink-700">
                      <strong className="text-ink-950">Winter:</strong> Bora-Wind kann eisig werden. Pfotenpflege bei Streusalz, Indoor-Tricks bei {'<'} -5 °C.
                    </span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <Bird className="w-4 h-4 mt-0.5 text-lake-600 shrink-0" />
                    <span className="text-ink-700">
                      <strong className="text-ink-950">Frühjahr:</strong> Vogelzug am See — höchste Reizdichte. Ideal für Impulskontroll-Training.
                    </span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <Sparkles className="w-4 h-4 mt-0.5 text-brand-600 shrink-0" />
                    <span className="text-ink-700">
                      <strong className="text-ink-950">Herbst:</strong> Beste Trainingszeit — milde Temperaturen, Weinlese, weniger Touristen.
                    </span>
                  </li>
                </ul>
              </div>
