    ...
    <Link
      ...
      className={`group inline-flex items-center gap-2.5 rounded-full pl-2.5 pr-4 py-2 border transition-all ${
        isDark
          ...
          : 'bg-white/85 border-ink-200 backdrop-blur shadow-[0_8px_24px_-12px_rgba(0,0,0,0.15)] hover:border-lake-400 hover:shadow-[0_12px_28px_-12px_rgba(0,0,0,0.2)]'
      } ${className}`}
    >
      ...
      <span className="flex flex-col leading-none">
        ...
        <span
          className={`mt-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${
            isDark ? 'text-cream/70 group-hover:text-cream' : 'text-ink-500 group-hover:text-lake-700'
          } transition-colors`}
        >
